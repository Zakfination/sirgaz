...
export const upsertProfile = async (userId, patch) => {
  const sb = getSupabase(); if (!sb) return { error: "no client" };
  const { data, error } = await sb.from("profiles").upsert({ id: userId, ...patch }).select().single();
...
export const getMyVenue = async (userId) => {
  const sb = getSupabase(); if (!sb) return null;
  const { data } = await sb.from("venues").select("*").eq("owner_id", userId).maybeSingle();
  return data;
};
export const createVenue = async (userId, patch) => {
  const sb = getSupabase(); if (!sb) return { error: "no client" };
  const { data, error } = await sb.from("venues").insert({ owner_id: userId, ...patch }).select().single();
  return { data, error: error?.message };
};
export const updateVenue = async (venueId, patch) => {
  const sb = getSupabase(); if (!sb) return { error: "no client" };
  const { data, error } = await sb.from("venues").update(patch).eq("id", venueId).select().single();
  return { data, error: error?.message };
};
...
export const listEvents = async (venueId) => {
  ...
  const { data } = await sb.from("events").select("*").eq("venue_id", venueId).order("created_at", { ascending: false });
  return data || [];
};
export const listPublishedEvents = async () => {
  const sb = getSupabase(); if (!sb) return [];
  const { data } = await sb.from("events").select("*, venues(name)").in("status", ["published", "live"]).order("starts_at", { ascending: true });
  return data || [];
};
export const getEvent = async (id) => {
  const sb = getSupabase(); if (!sb) return null;
  const { data } = await sb.from("events").select("*, venues(name, address)").eq("id", id).maybeSingle();
  return data;
};
export const createEvent = async (venueId, patch) => {
  const sb = getSupabase(); if (!sb) return { error: "no client" };
  const { data, error } = await sb.from("events").insert({ venue_id: venueId, ...patch }).select().single();
  return { data, error: error?.message };
};
...
export const joinEvent = async (eventId, userId) => {
  const sb = getSupabase(); if (!sb) return { error: "no client" };
  const { data, error } = await sb.from("event_participants").upsert({ event_id: eventId, user_id: userId, status: "waiting" }, { onConflict: "event_id,user_id" }).select().single();
  return { data, error: error?.message };
};
export const listParticipants = async (eventId) => {
  const sb = getSupabase(); if (!sb) return [];
  const { data } = await sb.from("event_participants").select("*, profiles!inner(*)").eq("event_id", eventId).order("joined_at", { ascending: true });
  return data || [];
};
export const countParticipants = async (eventId) => {
  const sb = getSupabase(); if (!sb) return 0;
  const { count } = await sb.from("event_participants").select("*", { count: "exact", head: true }).eq("event_id", eventId);
  return count || 0;
};
...
export const findOrCreateMatch = async (eventId, meId) => {
  const sb = getSupabase(); if (!sb) return null;
  // 1. Check existing match for me at this event
  const { data: existing } = await sb.from("matches").select("*").eq("event_id", eventId).or(`user_a.eq.${meId},user_b.eq.${meId}`).limit(1).maybeSingle();
  if (existing) return existing;

  // 2. Get all participants of this event with profiles
  const parts = await listParticipants(eventId);
  const me = parts.find(p => p.user_id === meId);
  if (!me) return null;
  const candidates = parts.filter(p => p.user_id !== meId);
  if (candidates.length === 0) return null;

  // 3. Pick best by compat
  let best = null; let bestOut = null;
  for (const c of candidates) {
    const r = computeVibeMatch(me.profiles, c.profiles);
    if (!best || r.score > best.score) { best = { score: r.score, breakdown: { ...r.breakdown, reasons: r.reasons }, user_b: c.user_id }; bestOut = c; }
  }
  // 4. Insert match
  const { data: created } = await sb.from("matches").insert({
    event_id: eventId, user_a: meId, user_b: best.user_b, score: best.score, breakdown: best.breakdown
  }).select().single();
  return created;
};
...
export const getOrCreateMission = async (matchId) => {
  const sb = getSupabase(); if (!sb) return null;
  const { data: existing } = await sb.from("missions").select("*").eq("match_id", matchId).maybeSingle();
  if (existing) return existing;
  const tmpl = MISSION_TEMPLATES[Math.floor(Math.random() * MISSION_TEMPLATES.length)];
  const { data } = await sb.from("missions").insert({ match_id: matchId, ...tmpl }).select().single();
  return data;
};

export const updateMission = async (id, patch) => {
  const sb = getSupabase();
  const { data } = await sb.from("missions").update(patch).eq("id", id).select().single();
  return data;
};
...
export const createReward = async (userId, eventId, missionId, patch) => {
  const sb = getSupabase();
  const { data, error } = await sb.from("rewards").insert({ user_id: userId, event_id: eventId, mission_id: missionId, ...patch }).select().single();
  return { data, error: error?.message };
};
export const listMyRewards = async (userId) => {
  const sb = getSupabase(); if (!sb) return [];
  const { data } = await sb.from("rewards").select("*, events(title)").eq("user_id", userId).order("created_at", { ascending: false });
  return data || [];
};
export const redeemReward = async (code) => {
  const sb = getSupabase();
  const { data, error } = await sb.from("rewards").update({ redeemed_at: new Date().toISOString() }).eq("code", code).select().single();
  return { data, error: error?.message };
};
